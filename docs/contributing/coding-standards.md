...
NHS.Wales frontend uses the Block Element Modifier (BEM) methodology when naming
...
We have chosen as Nunjucks as the templating language for NHS.Wales frontend components. We expose those templates as reusable chunks of code: macros. Developers import macros into their application, call them as per documentation and provide data to its arguments.
...
You can find NHS.Wales frontend components in `packages/components`.
